...
class CrtsBuilder(builders.LLVMRuntimeBuilder):
    name: str = 'clangrt'
    ...
    def cmake_defines(self) -> Dict[str, str]:
        ...
        defines['COMPILER_RT_TEST_TARGET_TRIPLE'] = arch.android_triple
        defines['COMPILER_RT_INCLUDE_TESTS'] = 'OFF'
        defines['SANITIZER_CXX_ABI'] = 'libcxxabi'
        libs: List[str] = []
        if arch == 'arm':
            libs += ['-latomic']
        if self._config._api_level < 21:
    ...
    def install_config(self) -> None:
        ...
        if self._config.platform:
            # Renames to lib/linux for backwards compatibility.
            dst_dir = self.install_dir / 'lib' / 'linux'
            shutil.copytree(lib_dir, dst_dir, dirs_exist_ok=True, copy_function=shutil.move)
...
class LibOMPBuilder(builders.LLVMRuntimeBuilder):
    ...
    config_list: List[configs.Config] = (
        configs.android_configs(platform=True, ext_config={'is_shared': False}) +
        configs.android_configs(platform=False, ext_config={'is_shared': False}) +
        configs.android_configs(platform=False, ext_config={'is_shared': True})
    )

    @property
    def is_shared(self) -> bool:
        return cast(Dict[str, bool], self._config.ext_config)['is_shared']
...
def build_runtimes(toolchain, args=None):
    ...
    CrtsBuilder().build()
    if BuilderRegistry.should_build('clangrt'):
