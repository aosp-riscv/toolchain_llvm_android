...
class Stage1Builder(builders.LLVMBuilder):
    ...
    @property
    def llvm_targets(self) -> Set[str]:  # type: ignore
    ...
    @property
    def llvm_projects(self) -> Set[str]:  # type: ignore
...
def set_lldb_flags(install_dir, host: hosts.Host, defines, env=None):
    if host.is_windows:
        build_os = 'linux-x86'
    else:
...
class Stage2Builder(builders.LLVMBuilder):
    ...
    def cmake_defines(self) -> Dict[str, str]:
        ...
        if self.build_lldb:
            set_lldb_flags(str(self.install_dir), hosts.build_host(), defines)
