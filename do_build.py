...
def prepare_wintoolchain(win_toolchain_path):
    WIN_SDK_VER = '10.0.18362.0'
    lib_path = win_toolchain_path / 'win_sdk' / 'Lib' / WIN_SDK_VER / 'um' / 'x64'
    for f in lib_path.iterdir():
        f.rename(lib_path / f.name.lower())
    create_symlink(win_toolchain_path / 'win_sdk' / 'Include' / WIN_SDK_VER / 'shared' / 'driverspecs.h', 'DriverSpecs.h')
    create_symlink(win_toolchain_path / 'win_sdk' / 'Include' / WIN_SDK_VER / 'shared' / 'specstrings.h', 'SpecStrings.h')
    create_symlink(win_toolchain_path / 'win_sdk' / 'Include' / WIN_SDK_VER / 'shared' / 'WTypesbase.h', 'wtypesbase.h')
    create_symlink(win_toolchain_path / 'win_sdk' / 'Include' / WIN_SDK_VER / 'shared' / 'WTypesbase.h', 'wtypesbase.h')
...
def build_llvm_for_windows(enable_assertions,
    ...
    if windows_toolchain:
        prepare_wintoolchain(windows_toolchain)
