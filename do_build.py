...
def get_sysroot(arch: hosts.Arch, platform=False):
    ...
    return os.path.join(sysroots, platform_or_ndk, arch.to_ndk_arch())
...
def create_sysroots():
    ...
    for (arch, target) in configs:
        ...
        for platform_or_ndk in ['platform', 'ndk']:
            ...
            # Copy over usr/lib.
            arch_lib_path = os.path.join(base_lib_path, 'arch-' + arch.to_ndk_arch(),
                                         'usr', 'lib')
            ...
            if arch == hosts.Arch.X86_64:
                arch_lib64_path = os.path.join(base_lib_path, 'arch-' + arch.to_ndk_arch(),
                                               'usr', 'lib64')
            ...
            if platform:
                ...
                platform_stubs = utils.out_path('platform_stubs', arch.to_ndk_arch())
...
def cross_compile_configs(toolchain, platform=False, static=False):
    ...
    for (arch, toolchain_path, llvm_triple, extra_flags) in configs:
        ...
        if arch == hosts.Arch.ARM:
            ...
            toolchain_lib = ndk_toolchain_lib(arch, arch.to_ndk_arch() + '-4.9',
                                              llvm_triple)
        else:
