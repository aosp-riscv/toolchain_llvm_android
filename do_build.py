...
def build_llvm_for_windows(enable_assertions: bool,
    ...
    if BUILD_LLDB:
        ...
        win_builder.libedit_path = None
        win_builder.swig_executable = swig_builder.install_dir / 'bin' / 'swig'
        xz_builder = builders.XzBuilder(config_list)
        xz_builder.build()
        win_builder.liblzma_path = xz_builder.install_dir
        libxml2_builder = builders.LibXml2Builder(config_list)
        libxml2_builder.build()
        win_builder.libxml2_path = libxml2_builder.install_dir
        lldb_deps_to_install.append(
            paths.get_libxml2_dynamic_lib(libxml2_builder.install_dir, hosts.Host.Windows))
...
def main():
    ...
    if need_host:
        ...
        if BUILD_LLDB:
            ...
            xz_builder.build()
            stage2.liblzma_path = xz_builder.install_dir
            libxml2_builder = builders.LibXml2Builder()
            libxml2_builder.build()
            lldb_deps.append(paths.get_libxml2_lib(libxml2_builder.install_dir, hosts.build_host()))
            stage2.libxml2_path = libxml2_builder.install_dir
            libedit_builder = builders.LibEditBuilder()
            libedit_builder.build()
            stage2.libedit_path = libedit_builder.install_dir
            lldb_deps.append(paths.get_libedit_lib(libedit_builder.install_dir, hosts.build_host()))
