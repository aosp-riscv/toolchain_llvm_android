...
def build_llvm_for_windows(stage1_install,
    ...
    if BUILD_LLDB:
        ...
        LLVMBuilder.set_lldb_flags(hosts.Host.Windows, windows_extra_defines)
...
def install_lldb_deps(install_dir, host: hosts.Host):
    ...
    if host.is_linux:
        os.symlink('../python3/lib/libpython3.8.so.1.0', os.path.join(lib_dir, 'libpython3.8.so.1.0'))
        shutil.copy2(paths.get_libedit_lib(host), lib_dir)
    elif host.is_windows:
        os.symlink('../python3/python38.dll', os.path.join(install_dir, 'bin', 'python38.dll'))
    elif host.is_darwin:
        os.symlink('../python3/lib/libpython3.8.dylib', os.path.join(lib_dir, 'libpython3.8.dylib'))
