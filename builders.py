...
class LLVMRuntimeBuilder(LLVMBaseBuilder):  # pylint: disable=abstract-method
    ...
    @property
    def install_dir(self) -> Path:
        config = cast(configs.AndroidConfig, self._config)
        if config.platform:
            path = self.toolchain.resource_dir
        else:
            path = self.toolchain.path / 'runtimes_ndk_cxx'
        return path / config.target_arch.value
