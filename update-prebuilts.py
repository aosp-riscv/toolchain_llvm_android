...
def create_rbe_inputs_file(install_dir, llvm_version):
    print(install_dir)
    inputs_file = open(os.path.join(install_dir, 'bin', 'remote_toolchain_inputs'), 'w')
    dependencies = ('clang\n'
                    'clang++\n'
                    'clang.real\n'
                    'clang++.real\n'
                    '../lib64/libc++.so.1\n'
                    '../lib64/clang/' + llvm_version + '/share/cfi_blacklist.txt\n'
                   )
