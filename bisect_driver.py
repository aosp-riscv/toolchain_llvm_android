...
def lock_file(path, mode):
    ...
    to also acquire lock through this method. If only reading (modes 'r' and
    'rb') then the lock is shared (i.e. many reads can happen concurrently, but
    only one process may write at a time).

    This function is a contextmanager, meaning it's meant to be used with the
    "with" statement in Python. This is so cleanup and setup happens
    automatically and cleanly. Execution of the outer "with" statement happens
    at the "yield" statement. Execution resumes after the yield when the outer
    "with" statement ends.
...
def get_dep_path(execargs):
    ...
    Returns:
      Absolute path of dependency file path from execution args (-o argument).
      If no dependency being outputted then return empty string.
    """
...
def bisect_triage(execargs, bisect_dir):
    ...
    if not in_object_list(full_obj_path, obj_list):
        if CONTINUE_ON_MISSING:
            ...
            raise Error(('%s is missing from cache! To ignore export '
                         'BISECT_CONTINUE_ON_MISSING=1. See documentation for '
                         'more details on this option.' % full_obj_path))

    cache = which_cache(full_obj_path)

    # If using safe WRAPPER_SAFE_MODE option call compiler and overwrite the
    # result from the good/bad cache. This option is safe and covers all
    # compiler side effects, but is very slow!
    ...
    # If generated object file happened to be pruned/cleaned by Make then link
    # it over from cache again.
