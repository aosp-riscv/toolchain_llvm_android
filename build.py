...
def build_llvm_for_windows(stage1_install,
    ...
    set_lldb_flags('windows-x86', windows_extra_defines, windows_extra_env)
...
def set_lldb_flags(host, defines, env):
    if host == 'windows-x86':
        build_os = 'linux-x86'
    else:
        build_os = host
    ...
    if host != 'windows-x86-64':
        libedit_root = utils.android_path('prebuilts', 'libedit', host)
        defines['libedit_INCLUDE_DIRS'] = os.path.join(libedit_root, 'include')
        defines['libedit_LIBRARIES'] = os.path.join(libedit_root, 'lib', 'libedit' + shlib_ext)
...
def build_stage2(stage1_install,
    ...
    set_lldb_flags(utils.build_os_type(), stage2_extra_defines, stage2_extra_env)
