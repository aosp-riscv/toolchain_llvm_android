...
def parse_args():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Don\'t build toolchain for Linux (and Android runtimes)')

    parser.add_argument(
        '--no-build-windows',
        action='store_true',
        default=False,
        help='Don\'t build toolchain for 32-bit and 64-bit Windows')

    parser.add_argument(
        '--no-build-windows-x86',
        action='store_true',
        default=False,
        help='Don\'t build toolchain for 32-bit Windows')

    parser.add_argument(
        '--no-build-windows-x86-64',
        action='store_true',
        default=False,
        help='Don\'t build toolchain for 64-bit Windows')
...
def main():
    ...
    need_host = utils.host_is_darwin() or not args.no_build_linux
    need_windows_32 = utils.host_is_linux() and not args.no_build_windows_x86 \
        and not args.no_build_windows
    need_windows_64 = utils.host_is_linux() and not args.no_build_windows_x86_64 \
        and not args.no_build_windows
