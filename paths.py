...
def get_python_lib_dir(host: hosts.Host) -> Path:
    """Returns the path to python lib dir for a host."""
    python_root = get_python_dir(host)
    return {
        hosts.Host.Linux: python_root / 'lib' / 'libpython3.8.so',
        hosts.Host.Darwin: python_root / 'libs' / 'python38.lib',
        hosts.Host.Windows: python_root / 'lib' / 'libpython3.8.dylib',
    }[host]
