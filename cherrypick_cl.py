...
def generate_patch_files(sha_list: List[str], start_version: int) -> PatchList:
    ...
    # Fetch upstream may take a long time. So print something.
    print('fetch upstream ...')
    fetch_upstream(upstream_dir)
    result = PatchList()
    for sha in sha_list:
        file_path = paths.SCRIPTS_DIR / f'patches/cherry/{sha}.patch'
        assert not file_path.exists(), f'{file_path} already exists'
        output = check_output(f'git format-patch -1 {sha} --stdout', shell=True, cwd=upstream_dir)
