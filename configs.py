...
class _BaseConfig(Config):  # pylint: disable=abstract-method
    """Base configuration for all host targets."""
...
class DarwinConfig(_BaseConfig):
    ...
    host: hosts.Host = hosts.Host.Darwin
    use_lld: bool = False
...
class LinuxConfig(_GccConfig):
    ...
    host: hosts.Host = hosts.Host.Linux
    gcc_root: Path = (paths.ANDROID_DIR / 'prebuilts' / 'gcc' / host.os_tag /
    ...
    @property
    def sysroot(self) -> Optional[Path]:
        return (paths.ANDROID_DIR / 'prebuilts' / 'gcc' / self.host.os_tag /
...
class WindowsConfig(_GccConfig):
    ...
    host: hosts.Host = hosts.Host.Windows
    root: Path = (paths.ANDROID_DIR / 'prebuilts' / 'gcc' / host.os_tag /
