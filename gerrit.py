...
def gerrit_query_change(query: str):
    ...
    output = utils.check_output(['gob-curl', '--request', 'GET',
                                ...
                                stderr=None) # Hide curl transfer stats
...
class SoongSwitchoverChange(GerritChange):
    ...
    def upload_change(self):
        ...
        message_lines = [
            ...
            # prebuilts for the same clang_revision.
            f'[DO NOT SUBMIT] Switch to clang {self.clang_revision} ({self.clang_build}).',
            '',
            'For testing',
            'Test: N/A'
        ]
...
def get_soong_change(build, clang_info) -> SoongSwitchoverChange:
    return SoongSwitchoverChange(build, clang_info)


def get_prebuilt_change(build) -> ClangPrebuiltChange:
