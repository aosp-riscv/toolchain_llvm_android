...
def build_kythe_corpus(builder: builders.Stage2Builder) -> None:
    ...
    extractor = utils.subprocess_run([paths.KYTHE_RUN_EXTRACTOR,
        'compdb',
        '-extractor=' + paths.KYTHE_CXX_EXTRACTOR,
        f'-path={json}'],
        env=env,
        capture_output=True)
    ...
    rel_path = lambda full_path: full_path[len(str(paths.ANDROID_DIR))+1:]
    srcs = re.findall('(?P<file>\S+)\': error running extractor',
                      extractor.stderr)
    srcSet = set(rel_path(f) for f in srcs)
