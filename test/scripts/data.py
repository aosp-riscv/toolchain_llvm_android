...
class ForrestPendingTable(CSVTable[ForrestPendingRecord]):
    ...
    def addInvocation(self,
        ...
        row = self.get(lambda that: that.invocation_id == record.invocation_id)
        if row:
    ...
    def find(self, prebuilt_build_number, tag, branch,
             target) -> Optional[ForrestPendingRecord]:
    ...
    def findByInvocation(self, invocation_id) -> Optional[ForrestPendingRecord]:
        """Find Forrest invocation based on invocation_id."""
        return self.getOne(lambda that: that.invocation_id == invocation_id)


class ForrestTable(CSVTable[ForrestRecord]):
    """CSV table for bookkeeping pending Forrest invocations."""
    makeRow = ForrestRecord._make

    def addInvocation(self, record: ForrestRecord, writeBack=True) -> None:
        """Add invocation to CSV Table and optionally write back."""
        row = self.get(lambda that: that.invocation_id == record.invocation_id)
        if row:
            raise RuntimeError(f'Invocation {record} already exists')
    ...
    def findByInvocation(self, invocation_id) -> Optional[ForrestRecord]:
        """Find Forrest invocation based on invocation_id."""
        return self.getOne(lambda that: that.invocation_id == invocation_id)
